        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {features.map((f) => (
              <div
                key={f.title}
                className="bg-white rounded-xl p-8 shadow-sm hover:shadow-md transition-shadow border border-gray-100 flex flex-col gap-4"
              >
                <span className="text-4xl">{f.icon}</span>
                <h3 className="font-oswald text-xl font-semibold text-gray-800 tracking-wide">
                  {f.title}
                </h3>
                <p className="font-unna text-gray-500 leading-relaxed">{f.desc}</p>
              </div>
            ))}
          </div>
        </div>
