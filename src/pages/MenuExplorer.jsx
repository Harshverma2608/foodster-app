...
export default function MenuExplorer() {
  ...
  const filtered = meals.filter(m =>
    (activeCategory === 'All Meals' || m.category === activeCategory) &&
  ...
  return (
    <div className="bg-[#f8f9ff] min-h-screen font-geist">
      <AuthNavbar />
      <main className="max-w-[1280px] mx-auto px-5 md:px-16 pt-28 pb-24">
        {/* Hero Search */}
        <section className="py-10 md:py-20 text-center max-w-3xl mx-auto">
          <h1 className="font-bold text-4xl md:text-[56px] text-[#0b1c30] mb-6" style={{ fontFamily: 'Montserrat, sans-serif', lineHeight: 1.1, letterSpacing: '-0.02em' }}>
            Discover Your Next <span className="text-[#b61722]">Favorite Meal.</span>
          </h1>
          ...
          <div className="relative">
            ...
            <input
              className="w-full pl-14 pr-6 py-4 md:py-5 rounded-full border border-[#8f6f6d]/30 bg-white shadow-sm focus:outline-none focus:ring-2 focus:ring-[#b61722]/20 focus:border-[#b61722] transition-all text-sm md:text-base"
              placeholder="Search meals, ingredients, dietary preferences..."
              value={search}
          ...
          <div className="flex items-center gap-3 overflow-x-auto pb-3 no-scrollbar">
            {categories.map(cat => (
              <button
                ...
                className={`flex items-center gap-2 px-5 py-2.5 rounded-full text-sm font-semibold whitespace-nowrap transition-all ${activeCategory === cat ? 'bg-[#b61722] text-white shadow-md' : 'bg-[#eff4ff] text-[#5b403e] hover:bg-[#dce9ff]'}`}
              >
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-24">
            ...
            <div key={meal.id} className="group flex flex-col bg-white rounded-xl overflow-hidden shadow-[0_4px_20px_rgba(0,0,0,0.04)] border border-[#8f6f6d]/10 transition-all hover:shadow-[0_12px_30px_rgba(0,0,0,0.08)] hover:-translate-y-1">
              <div className="relative h-64 overflow-hidden">
                ...
                <div className="absolute top-4 left-4 flex flex-wrap gap-2">
                  {meal.tags.map((tag, i) => (
                    <span key={tag} className={`px-3 py-1 backdrop-blur-md rounded-full text-xs font-medium ${meal.tagClasses[i]}`}>{tag}</span>
                  ))}
                </div>
              </div>
              <div className="p-6 flex flex-col flex-grow">
                <div className="flex justify-between items-start mb-2">
                  <h3 className="font-bold text-lg text-[#0b1c30]" style={{ fontFamily: 'Montserrat, sans-serif' }}>{meal.name}</h3>
                  <span className="font-semibold text-sm text-[#b61722] ml-2 flex-shrink-0">{meal.price}</span>
                </div>
                ...
                <div className="mt-auto pt-5 border-t border-[#8f6f6d]/10 flex items-center justify-between">
                  ...
                  <Link to="/plans">
                    <button className="flex items-center gap-2 px-5 py-2 bg-gradient-to-r from-orange-500 to-[#b61722] text-white rounded-full text-sm font-semibold shadow-sm hover:shadow-md active:scale-95 transition-all">
                      Add to Plan
                    </button>
                  </Link>
                </div>
          ...
          {filtered.length === 0 && (
            <div className="col-span-3 text-center py-20 text-[#5b403e]">
