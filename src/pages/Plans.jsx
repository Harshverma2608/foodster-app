...
export default function Plans() {
  return (
    <div className="bg-[#f8f9ff] min-h-screen font-geist">
      ...
      <main>
        {/* Hero */}
        <section className="pt-32 pb-16 px-5 md:px-16 text-center max-w-[1280px] mx-auto">
          <h1 className="font-bold text-4xl md:text-[56px] text-[#0b1c30] mb-4" style={{ fontFamily: 'Montserrat, sans-serif', lineHeight: 1.1, letterSpacing: '-0.02em' }}>Choose Your Perfect Plan</h1>
        ...
        {/* Plan Cards */}
        <section className="px-5 md:px-16 pb-24 max-w-[1280px] mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-start">
            {plans.map(plan => (
              <div key={plan.id} className={`relative bg-white border ${plan.borderClass} rounded-xl p-8 shadow-[0_4px_20px_rgba(0,0,0,0.04)] flex flex-col transition-all hover:shadow-xl ${plan.highlight ? 'md:scale-105 z-10 shadow-xl' : ''}`}>
                {plan.badge && (
                ...
                <div className="mb-6">
                  <span className={`px-3 py-1 rounded-full text-xs font-semibold ${plan.labelClass}`}>{plan.label}</span>
                </div>
                <h3 className="font-bold text-2xl text-[#0b1c30] mb-2" style={{ fontFamily: 'Montserrat, sans-serif' }}>{plan.name}</h3>
                ...
                <Link to="/checkout">
                  <button onClick={() => localStorage.setItem('activePlan', 'true')} className={`w-full py-4 rounded-xl font-semibold text-sm transition-all active:scale-95 ${plan.btnClass}`}>Select Plan</button>
                </Link>
              </div>
            ))}
          </div>
        ...
        {/* Custom Plan */}
        <section className="bg-[#eff4ff] py-24 px-5 md:px-16">
          <div className="max-w-[1280px] mx-auto flex flex-col md:flex-row gap-16 items-center">
            <div className="w-full md:w-1/2">
              <h2 className="font-bold text-3xl text-[#0b1c30] mb-6" style={{ fontFamily: 'Montserrat, sans-serif' }}>Build Your Custom Plan</h2>
              <p className="text-lg text-[#5b403e] mb-8 leading-relaxed">Need more flexibility? Create a meal schedule that fits your unique lifestyle. Choose your delivery days, calorie goals, and portion sizes precisely how you want them.</p>
            ...
            <div className="w-full md:w-1/2 relative mt-8 md:mt-0">
              ...
              <div className="absolute -bottom-4 left-4 md:-bottom-6 md:-left-6 bg-white p-5 rounded-2xl shadow-2xl flex items-center gap-4">
        ...
        {/* FAQ CTA */}
        <section className="py-24 px-5 md:px-16 text-center">
          <h2 className="font-bold text-2xl text-[#0b1c30] mb-4" style={{ fontFamily: 'Montserrat, sans-serif' }}>Still have questions?</h2>
